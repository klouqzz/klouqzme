...
export default function Projects() {
    ...
    <div className="px-6 lg:px-36 h-full py-36 pt-24">
        ...
        <p style={{ zIndex: 2 }} className="relative ml-2 text-3xl lg:text-6xl font-semibold dark:text-gray-300">Projects</p>
          <p className="hidden lg:block absolute bottom-5 opacity-10 ml-2 text-8xl font-semibold dark:text-white">Projects</p>
                    ...
                    projects?.map((p, i) => (
                        <a href={p?.link} target="_blank" key={i} className="cursor-pointer w-full bg-zinc-100/90 p-4 rounded-lg hover:scale-[1.02] transition-all duration-200 hover:-translate-y-1 hover:bg-zinc-200/50 dark:bg-[rgba(0,0,0,.7)] dark:border-2 dark:border-solid dark:border-transparent dark:hover:border-cIndigo">
                            ...
                            <div className="mt-4">
                                ...
                                <p className="text-sm max-h-24 h-24 overflow-auto font-normal dark:text-gray-300">{p?.description}</p>
                            </div>

                            <p className="text-xs italic mt-6 dark:text-gray-400"><i className="fal fa-globe mr-1" />{p?.link}</p>
                        </a>
                    ))
                ) : <></>
